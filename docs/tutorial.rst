...
.. doctest::
   ...
   >>> accel = 1.3 * ureg['meter/second**2']
   >>> # Pretty print
   >>> print(u'The pretty representation is {:P}'.format(accel))
...
The `u` string prefix is needed in Python 2 to avoid a `UnicodeEncodeError` when printing unicode superscripts.
...
Finally, you can specify a default format specification:
   ...
   >>> print(u'The acceleration is {}'.format(accel))
